...
size_t height(const binary_tree_t *tree)
{
	if (tree)
	{
		...
		l = tree->left ? val_1 + height(tree->left) : val_1;
		r = tree->right ? val_1 + height(tree->right) : val_1;
		return ((l > r) ? l : r);
	}
	return (val_0);
}
...
int is_avl_helper(const binary_tree_t *tree, int lo, int hi)
{
	...
	if (tree != NULL)
	{
		...
		if (diff > val_1)
			return (val_0);
		return (is_avl_helper(tree->left, lo, tree->n - val_1) &&
			is_avl_helper(tree->right, tree->n + val_1, hi));
	}
	return (val_1);
}
