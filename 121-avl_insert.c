...
int balance(const binary_tree_t *tree)
{
	return (tree != NULL ? height(tree->left) - height(tree->right) : val_0);
}
		...
		avl_t **new, int value)
{
	...
	if (bfactor > val_1 && (*tree)->left->n > value)
		*tree = binary_tree_rotate_right(*tree);
	else if (bfactor < - val_1 && (*tree)->right->n < value)
		*tree = binary_tree_rotate_left(*tree);
	else if (bfactor > val_1 && (*tree)->left->n < value)
	{
